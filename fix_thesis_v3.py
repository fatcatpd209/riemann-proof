...
def replace_range(text, start_marker, end_marker, new_content, label):
    ...
    if i1 < 0:
        print('WARN %s start not found: %s' % (label, repr(start_marker[:60])))
        return text
...
# 1. FIX 4.1.3
# ============================================================
s413_new = """### 4.1.3 De Bruijn-Newman 三类变分框架（修正版）
...
**变分框架 1：Polya 实零判别（偶数整函数 Fourier 判据）**
Polya 1927：若 $H(z) = \int_0^\infty \Phi(u) \cos(zu) du$ 是偶函数整函数，且 $\Phi \in L^1(\mathbb{R}, (1+u^2)du)$ 满足 $\Phi(u) \ge 0$，则 $H$ 的所有零点实当且仅当 $\Phi$ 是**正测度的 Fourier 余弦变换**（Bochner 定理 + de Bruijn 实零判别）。对 $H_t$，热因子 $e^{tu^2}$ 改写为 $\Phi_t(u) = e^{tu^2}\Phi_0(u)$，要求 $\Phi_t$ 为偶正可测函数的 Fourier 余弦像。

**变分框架 2：Csordas-Smith-Varga Sturm-Liouville 振荡（1994）**
对任意 $z \in \mathbb{C}$，定义 Sturm-Liouville 微分算子：
$$\mathcal{A}f = -f'' + Q_{t}(u)\,f$$
其中 $Q_t(u) = -(\Phi_t'/\Phi_t)'(u) = \partial_{uu}\log |\Phi_t(u)|$ 是 $\Phi_t$ 诱导的势。$H_t$ 所有零点实当且仅当对每个 $\gamma \in \mathbb{R}$，方程 $\mathcal{A}f = \gamma^2 f$ 仅有实特征值（Sturm 比较定理 + 振荡计数）。**注**：这不是谐振子。

**变分框架 3：Dobner Dirichlet 级数零点判别（2020）**
$\zeta_t(s) = \sum_{n=1}^\infty \exp\left(\frac{t}{4}\log^2 n\right) n^{-s}$。对 $t<0$，$\zeta_t(s)$ 有无穷多零点偏离临界线 $\Re(s)=1/2$，直接证明 $t \notin S$。Dobner 给出 Newman 猜想（$\Lambda \ge 0$）的全新证明。

**单调性（Polya 热流定理）**：若 $H_{t_0}$ 零点全实且 $t > t_0$，则 $H_t$ 零点全实。标准证明（Polya 1935 via de Bruijn 1950）：$\Phi_t = \mathcal{G}_{\sqrt{t-t_0}} * \Phi_{t_0}$（Gaussian 卷积），保持 Polya 实零条件，故 $t > t_0 \implies t \in S$。$S = [\Lambda, +\infty)$ 右闭区间。

**重要声明**：原稿中 $E(\lambda) = \inf \int |f'|^2 + \lambda u^2 |f|^2$ 是错误路径。$\Lambda \le 0 \iff RH$ 是**定义级恒等**，而非独立 "证明" 的命题（见 4.3）。
"""
...
# 2. FIX 4.2
# ============================================================
s42_new = """## 4.2 De Bruijn-Newman 常数与 RH 的等价：诚实现状
...
- **Rodgers-Tao 证明思路**：反设 $\Lambda < 0$，取 $t\in(\Lambda,0)$，分析 $H_t$ 零点动力学，推出 $H_0$ 零点满足局部算术进步分布（由热流形变保持的局部平衡），与 Montgomery 对关联估计矛盾。
...
# 3. FIX 4.3
# ============================================================
s43_new = """## 4.3 三段独立充要：$\boldsymbol{\Lambda = 0 \iff RH}$（定义级等价）
...
### 4.3.4 诚实结论
$\Lambda = 0$ 与 RH 是同一数学命题的不同语言。**证明 $\Lambda = 0$ 就是证明 RH**，没有捷径。DBN 框架提供了漂亮的 reformulation，但没有绕过 RH 的核心困难（证明 $\xi$ 零点全在临界线）。
"""
...
# 4. FIX 2.1.3
# ============================================================
s213_new = """### 2.1.3 Sturm-Liouville 算子谱与 DBN 零点关系（修正版）
...
#### 2.1.3.3 Polya 实零判别（Fourier 方向）
$H_t(z) = \mathcal{F}_c \Phi_t(z) = \int_0^\infty \Phi_t(u) \cos(zu) du$ 零点全实当且仅当 $\Phi_t$ 是某个**正测度的 Fourier 余弦变换**（Bochner 定理 + de Bruijn 1950 实零判别）。等价地，$\Phi_t$ 必须完全凸。

#### 2.1.3.4 零点与 $\zeta$ 零点的一一对应
$H_0(z) = \frac{1}{8}\xi(\frac{1}{2}+\frac{iz}{2})$ 是定义恒等。$H_0(\gamma) = 0 \iff \xi(\frac{1}{2}+i\gamma) = 0$ 对 $\gamma \in \mathbb{R}$。计重双射 $\{\gamma \in \mathbb{R} \mid H_0(\gamma)=0\} \leftrightarrow \{\varrho \mid \zeta(\varrho)=0, \Re(\varrho)=\frac{1}{2}\}$（Hadamard 乘积指数 1，单阶零点已验证前 $10^{14}$ 个）。

#### 2.1.3.5 Polya 单调性
若 $t_0 \in S$ 且 $t > t_0$，则 $t \in S$。**正确演化**（Polya 1935）：$H_{t_2}(z) = \int_0^\infty e^{(t_2-t_1)u^2} \Phi_{t_1}(u) \cos(zu) du$，**不是**原稿声称的 $e^{-\Delta t z^2/4} H_{t_1}(z)$（FATAL 3）。
...
# 5. FIX 4.4
# ============================================================
s44_new = """## 4.4 后置等价推论：$\boldsymbol{\Lambda \le 0 \iff}$ 无穷多 Lehmer 对

**硬隔离**：本节不在主干证明链中使用。

**修正**：原稿 Lehmer 判别泛函 $F(\gamma,\gamma') < 4/5$ 是 Csordas-Smith-Varga 1994 的预施瓦茨间隔判据。Rodgers-Tao 2020 恰恰利用 $t \to 0^-$ 时零点间隙趋于 $0$ 的 Lehmer 子列来证明 $\Lambda \ge 0$Lehmer 对是 $\Lambda$ 接近 $0$ 的数值征兆，而非 RH 的 "旁路" 证明。
...
若 $\Lambda = 0$（RH 成立），零点密度 $N(T) \sim \frac{T}{2\pi}\log T$，平均间隙 $\overline{\Delta}(T) \to 0$。对任意大 $T_0$，存在 $T > T_0$ 使得间隙 $\Delta < \frac{1}{\log T}$，代入 $F$ 渐近式得 $F < 4/5$。无穷子列 $\{T_k\}$ 延伸。

### 4.4.3 诚实反向结论
Lehmer 对存在对 $\Lambda > 0$ 也成立（Rodgers-Tao 2020 证明 $\Lambda \ge 0$ 时仍有无穷多 Lehmer 对）。故 $F < 4/5$ 不是 $\Lambda \le 0$ 的充分条件。Lehmer 对更像 $\Lambda \approx 0$ 的数值征兆。
"""
...
# 6. FIX 5
# ============================================================
s5_new = """## 第 5 章 全域反例穷尽（修正版）

本章枚举 DBN 框架内可能存在的反例类型，以当前已证数学结论为基础。

### 5.1 反例 1：$\boldsymbol{S \neq [\Lambda, +\infty)}$
Polya 1935 已证明单调性：若 $H_{t_0}$ 零点全实且 $t > t_0$，则 $H_t$ 零点全实。$\Lambda = \inf S$ 有限（Newman 1976，$-\infty < \Lambda \le 1/2$）。$S$ 右闭（零点极限由 Hadamard 乘积 + 一致收敛给出实零点连续形变）。

### 5.2 反例 2：$\boldsymbol{H_t}$ 零点分岔出复共轭对（$\Lambda > 0$ 的原因）
**这不是反例，而是 Rodgers-Tao 证明 $\Lambda \ge 0$ 的核心机制**：$t = \Lambda$ 处零点从实轴 "逃逸" 到上/下半复平面形成复共轭对 $\alpha\pm i\beta$，$\beta\neq 0$。分岔发生在边界 $t=\Lambda$ 处，Polya 单调性在 $t=\Lambda$ 处不闭。
...
$\Phi(u) > 0$（逐点正系数正项和）。$\Phi_t(u) = e^{tu^2}\Phi(u)$ 对 $t \ge 0$ 保持正。$t < 0$ 时 $e^{tu^2}$ 对大 $|u|$ 衰减，但 $\Phi$ 的超指数压制保证 $\Phi_t(u)$ 对所有 $t \in \mathbb{R}$ 在 $u \to \infty$ 时仍衰减。

### 5.6 反例 6：$\boldsymbol{\Lambda}$ 数值不唯一
$\Lambda$ 是 $S = \{t : H_t \text{零点全实}\}$ 的下确界，由所有零点位置唯一确定。Polymath 2019 有效渐近分析 + 有限高度 RH 验证给出上界；Dobner 2020 Dirichlet 级数给出 $\Lambda \ge 0$ 解析下界。
